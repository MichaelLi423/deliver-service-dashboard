...
- **十类快速记录**：搬迁批次、搬迁仪器、上门活动、开单记录、实际物流费用、验收报告、掉票、
  Ship-to 申请、损坏/维修事项、补齐进单核心资料，均可就近录入当前选中项目。
- **独立模块**：序列号地址更新、二维码申请（仪器服务 / 项目验收单 / 物流管理三类）。
...
| 渲染 / 界面 | Vitest + Testing Library | `npm test` | 访问门、四步向导、十类动作、掉票编辑/撤销、报表、键盘焦点与可访问性 |
...
5. **报表界面仅月份筛选**：区域、开单类型、运输公司、工程师、责任人等筛选维度在口径中保留，
   当前界面未开放；"月度掉票次数 / 物流费用合同占比 / 待补实际费用清单"未作为独立小节展示。
6. **二维码申请界面仅 3 类**：仪器服务、项目验收单、物流管理；历史数据中无法还原的类型
   无法通过界面补齐。
7. **提醒临期窗口固定默认 7 天**：领域层支持配置，界面暂无配置入口。
8. **开单记录无列表页**：只能录入，项目详情选项卡暂不展示开单记录列表。
9. **单一账号**：无多账号、注册、角色与权限管理；工作量归属唯一本地账号。
10. **不联网**：无云同步、无远程数据库、无外部渠道提醒（邮件/企业微信等）。
