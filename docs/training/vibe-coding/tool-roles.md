...
本课（90 分钟、28 页）工具**按职责讲，不按产品记**：先理解每类工具承担什么职责，具体产品只是实现该职责的示例。三套关键工具深入"是什么/为什么/怎么用"；MCP 只讲边界。页 8–11 使用静态重构材料，项目段打开已有历史会话讲解已有输出。版本一律"以官网/上游为准"，具体数字见 [environment-checklist.md](./environment-checklist.md)。
...
| 终端 | 运行测试、git、查看 diff | 系统终端 | 课前自检与静态材料说明；项目段不重跑历史命令 |
...
- 页 8–11 的 Agent 输出为静态重构材料；项目段打开 OpenCode 已有历史会话并讲解已有输出。
- 项目段不重新调用 Agent、不重新执行历史命令；历史会话打不开时使用已脱敏讲师笔记与现有证据摘要。
