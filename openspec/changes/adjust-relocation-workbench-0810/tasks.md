...
本文件按依赖顺序拆分实施任务，属规划工件，不包含代码实现。实施基准为 `proposal.md`（Why/What/Impact）、`design.md`（D1~D8 决策与迁移计划）与 `specs/**/spec.md`（12 份 delta 规格）。design.md 无未决 Open Questions（TBD-07/09/11 等均为对既有规格的交叉引用，不改变本次建设范围）。
