...
function writeImportedServiceOrder(
  ...
  if (existing) {
    assertTargetUnmodified(
      ...
      'service_orders',
      existing.id,
      serviceOrderSnapshotHash(db, existing.id),
    );
    db.prepare(
  ...
  upsertRecordAudit(
    ...
    id,
    order.sourceHash,
    serviceOrderSnapshotHash(db, id),
    nowIso,
  );
}
...
function writeImportedInvoice(
  ...
  if (existing) {
    ...
    assertTargetUnmodified(
      ...
      'invoices',
      existing.id,
      invoiceSnapshotHash(db, existing.id),
    );
    db.prepare(
...
function writeImportedSerialAddressUpdate(
  ...
  if (existing) {
    assertTargetUnmodified(
      ...
      existing.id,
      serialAddressUpdateSnapshotHash(db, existing.id),
    );
    db.prepare(
  ...
  upsertRecordAudit(
    ...
    id,
    update.sourceHash,
    serialAddressUpdateSnapshotHash(db, id),
    nowIso,
  );
}
...
function writeImportedShipToRequest(
  ...
  if (existing) {
    assertTargetUnmodified(
      ...
      'ship_to_requests',
      existing.id,
      shipToRequestSnapshotHash(db, existing.id),
    );
    db.prepare(
  ...
  upsertRecordAudit(
    ...
    id,
    req.sourceHash,
    shipToRequestSnapshotHash(db, id),
    nowIso,
  );
}
