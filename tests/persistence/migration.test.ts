...
import { bootstrapDatabase, INITIAL_MIGRATION } from '../../src/domain/capabilities/local-data-persistence/bootstrap';
...
describe('schema 迁移（tasks 1.10 / D17）', () => {
  it('成功迁移：v0 → v12 应用初始 schema、归属快照列、迁移来源列、导入记录审计、正式库身份与业务修订触发器、v2 读取索引并写入 user_version', () => {
    const dir = makeTempDir();
    try {
      const { db, migrationResult } = bootstrapDatabase({ dataDir: dir });
      expect(migrationResult.fromVersion).toBe(0);
      expect(migrationResult.toVersion).toBe(12);
      expect(migrationResult.applied).toHaveLength(12);
  ...
  it('幂等：已处于最新版本时不再重复迁移', () => {
    ...
    try {
      ...
      expect(readSchemaVersion(second.db)).toBe(12);
